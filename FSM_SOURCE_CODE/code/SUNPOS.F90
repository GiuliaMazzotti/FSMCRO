...
!!      G. Mazzotti          Sept 2022 adapt original subroutine for use within FSM-CRO
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
!
USE CRO_CSTS, ONLY : EXPI, EXDAY
USE CRO_GRID, ONLY : KSIZE1
!
!
!#ifdef AIX64
!$ USE OMP_LIB
...
REAL,         INTENT(IN)   :: PLON(KSIZE1)     ! longitude
REAL,           INTENT(IN)   :: PLAT(KSIZE1)       ! latutude
!
REAL,           INTENT(OUT)  :: PZENITH(KSIZE1)   ! Solar zenithal angle
...
IF( MOD(KYEAR,4).EQ.0 .AND. (MOD(KYEAR,100).NE.0 .OR. MOD(KYEAR,400).EQ.0)) THEN
  ZDATE = FLOAT(KDAY +   IBIS(KMONTH-1)) - 1 + PTIME/EXDAY
  ZAD = 2.0*EXPI*ZDATE/366.0
ELSE
  ZDATE = FLOAT(KDAY + INOBIS(KMONTH-1)) - 1 + PTIME/EXDAY
  ZAD = 2.0*EXPI*ZDATE/365.0
END IF

ZA1 = (1.00554*ZDATE- 6.28306)*(EXPI/180.0)
ZA2 = (1.93946*ZDATE+23.35089)*(EXPI/180.0)
...
!              ---------------------------
!
  ZLAT(JJ) = PLAT(JJ)*(EXPI/180.)
  ZLON(JJ) = PLON(JJ)*(EXPI/180.)
!
!-------------------------------------------------------------------------------
!
...
!              ----------------------------
!
  ZTUT(JJ) = ZUT - ZTSIDER + ZLON(JJ)*((180./EXPI)/15.0)
!
!
!-------------------------------------------------------------------------------
...
!              ---------------------------------------------------------
!
  ZSOLANG(JJ) = (ZTUT(JJ)-12.0)*15.0*(EXPI/180.)          ! hour angle in radians
!
