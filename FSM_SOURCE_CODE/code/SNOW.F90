...
use MODCONF, only: HYDROL,DENSTY,OSHDTN,HN_ON,SNFRAC,CRO_ON
...
do j = 1, Ny
do i = 1, Nx
if (crosnowmask(i,j) == 0) then

  Gsoil(i,j) = G(i,j)
  Roff(i,j) = 0
  meltflux_out(i,j) = 0
  Roff_bare(i,j) = Rf(i,j) * dt * (1 - fsnow(i,j))
...
if (crosnowmask(i,j) == 0) then
  ...
  ! in case of snowcro, this subroutine was entered only if snowfall was too small to build a snowpack
  ! therefore remove spurious snow and add it to runoff
  if (CRO_ON .AND. snowdepth > epsilon(snowdepth)) then 
    Roff(i,j) = Roff(i,j) + sum(Sice(:,i,j) + Sliq(:,i,j))
    Sice(1,i,j) = 0.
    Sliq(:,i,j) = 0.
    Ds(:,i,j)   = 0. 
    fsnow(i,j)  = 0.
