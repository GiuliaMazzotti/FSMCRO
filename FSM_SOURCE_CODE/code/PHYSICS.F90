...
use MODCONF, only: CANMOD, CRO_ON
...
! Identify points on which crocus should be run and run it
if (CRO_ON) then 
  call CRO_CHECKIN(crosnowmask,ISIZE_SNOW,NMASK)
  if (ISIZE_SNOW > 0) then
    call CRO_COUP(unload,ksoil,SWsrf,Rsrf,Hsrf,LEsrf,G,Gsoil, &
                meltflux_out,Roff,Sbsrf,SWsci,LWsci,Usc, & 
...
! enter FSM snow routine in any case but compute snowpack only where crosnowmax = 0
call SNOW(Esrf,G,ksnow,ksoil,Melt,unload,Gsoil,Roff,meltflux_out, & 
          Sbsrf,crosnowmask)
